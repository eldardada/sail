...
var btnCounter = /*#__PURE__*/function () {
  function btnCounter(owner) {
    _classCallCheck(this, btnCounter);
  ...
  _createClass(btnCounter, [{
    key: "startClickAnimation",
    value: function startClickAnimation() {
      this.owner.classList.add('active');
    }
  }, {
    key: "stopClickAnimation",
    value: function stopClickAnimation() {
      var _this = this;

      var time = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 80;
      setTimeout(function () {
        _this.owner.classList.remove('active');
      }, time);
    }
  }]);
...
function btnClickAnimation(parentBlock, btnsContainerSelector, btnSelector, btnIncSelector) {
  ...
  var interval;
  var intervalCount;
  parentBlock.addEventListener('input', function (e) {
    var target = e.target;
    target.value = target.value.replace(/\D/g, '');
  });
  parentBlock.addEventListener('change', function (e) {
    var target = e.target;
  ...
  parentBlock.addEventListener('mousedown', function (e) {
    var target = e.target;
    btnCount = target.closest(btnSelector);
    var countContainer = target.closest(btnsContainerSelector);
    var btnInc = countContainer.querySelector(btnIncSelector);
    ...
    if (btnCount && counterValidate(input)) {
      ...
      if (itInc) btn.startClickAnimation();else if (!itInc && input.value != 0) btn.startClickAnimation();
      var count = 0;
        ...
        if (count == 3) {
          if (itInc && input.value < 50) {
            interval = setInterval(function () {
              if (input.value < 50) input.value = Number(input.value) + 1;else if (input.value == 50) btn.stopClickAnimation();
            }, 100);
          } else if (!itInc && input.value > 0) {
            interval = setInterval(function () {
              if (input.value > 0) input.value -= 1;else if (input.value == 0) btn.stopClickAnimation();
            }, 100);
          }
        }
      }, 100);
  ...
  parentBlock.addEventListener('mouseup', function (e) {
    ...
    if (btnCount) {
      ...
      if (itInc && input.value < 50) input.value = Number(input.value) + 1;else if (!itInc && input.value > 0) input.value -= 1;
    }
  });
}
/**
...
if (aside) {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (!its_aside && !its_menu && aside_is_active) hideMenu();
  });
...
if (basket) {
  ...
  btnClickAnimation(basketModels, '.basket-bgmenu-count', '.btn_count', '.btn_count_inc');
}
...
if (card) {
  ...
  var cardCounterBlock = document.querySelector('.card-model__product-count');
    ...
    cardInfoAdd.addEventListener('click', function (e) {
      ...
      if (div.classList.contains('add__sign')) {
        ...
        if (mainParent.querySelector('.card-info-add__block-check input').checked) {
          ...
          var _input = div.querySelector('label>input');
          ...
          var itSpan = target == span || target == _input;
...
if (pay) {
  ...
  var payDataUser = document.querySelector('.pay-data__user');
  ...
  payDataUser.addEventListener('input', function (e) {
    var target = e.target;
    var id = target.id;
...
var simproducts = document.querySelector('.simproducts');
